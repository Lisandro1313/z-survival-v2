 ...
 * Migrado desde survival.html original - VERSION LIMPIA
 */
...
// Función auxiliar para obtener gameState
...
export const messageHandlers = {
    'player:data': (msg) => {
        if (msg.player) {
            ...
            console.log('✅ Datos del jugador actualizados:', getPlayer().nombre);
            renderGame();
        }
    },
    ...
    'world:state': (msg) => {
        setWorld(msg.world);
        console.log('📦 Mundo recibido:', msg.world);
    ...
    'craft:success': (msg) => {
        log(`🔨 Crafteaste: ${msg.item}`, 'success');
        const world = getWorld();
    ...
    'player:joined': (msg) => {
        worldLog(`👤 ${msg.nombre} se unió`, 'success');
        renderGame();
    },

    'player:left': (msg) => {
        worldLog(`👤 ${msg.nombre} se desconectó`, 'warning');
    },

    'players:list': (msg) => {
        console.log('🎮 Lista de jugadores recibida:', msg.players?.length || 0);
    ...
    'achievement:unlocked': (msg) => {
        showNotification(`🏆 ${msg.achievement.name}`, 'success');
        const player = getPlayer();
        if (player) {
    ...
    'mission:completed': (msg) => {
        showNotification(`✅ Misión: ${msg.mission?.descripcion}`, 'success');
        const player = getPlayer();
        const world = getWorld();
        if (msg.inventario && player) player.inventario = msg.inventario;
    ...
    'npc:talk': (msg) => {
        ...
        // Mostrar opciones de diálogo si existen
    ...
    'npc:mission_accepted': (msg) => {
        ...
        log(`📜 Misión aceptada: ${msg.mission.descripcion}`, 'success');
    ...
    'npc:mission_completed': (msg) => {
        ...
        if (msg.recompensas.reputacion) rewards.push(`+${msg.recompensas.reputacion} Reputación`);
        ...
        showNotification(`🎁 Recompensas: ${rewards.join(', ')}`, 'success');
        playSound('levelup');
        renderGame();
    },
    ...
    'quests': (msg) => {
        ...
        console.log('📋 Misiones recibidas:', allQuests.length);

        // Renderizar en la pestaña de misiones
            ...
            allQuests.forEach(q => {
                html += `
                    <div style="background: #1a2a1a; padding: 10px; margin: 5px 0; border-left: 3px solid #00ff00;">
                        ...
                        ${q.estado === 'disponible' ?
                        `<button onclick="window.acceptQuest('${q.id}')" style="padding: 5px 10px; margin-top: 5px;">✅ Aceptar</button>` :
    ...
    'error': (msg) => {
        log(`⚠️ ERROR: ${msg.error || msg.mensaje}`, 'warning');
        playSound('error');
    }
};
